...
object Props extends Logging {
  ...
  private def getPropertyFileInputStream: InputStream = {
    ...
    if (filePath != null && filePath.length > 0) {
      logger.info(s"config file：$filePath")
      if (Files.exists(Paths.get(filePath))) {
        is = new FileInputStream(filePath)
      } else {
        logger.info(s"在本地未找到config文件$filePath，尝试在HDFS上获取文件")
        val fs = FileSystem.get(new Configuration())
        if (fs.exists(new Path(filePath))) {
          val fis = fs.open(new Path(filePath))
          is = fis.getWrappedStream()
        } else {
          logger.info(s"在HDFS上找不到config文件$filePath，加载失败...")
        }
      }
    } else {
      logger.info(s"未设置配置文件PropPath")
    }
    is
  }
